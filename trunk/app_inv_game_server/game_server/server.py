...
arguments from the request, all operations are run as database transactions.
This means that any unhandled errors encountered during the operations
will result in the database performing a 'rollback' to the state that
it was in before the request was made.

All server command functions return a tuple of the database model they
operated on and a dictionary of results. These vary from command to
command, but all requests will provide their return value using an
...
responses as a web page. Put functions write json to the request handler
...
address will be used to identify players during game operations. These
same rules apply to other fields which identify players such as a new
leader or an invitee. In general, the variable name 'player' will be
used to represent values that are email addresses and pid is used more
generally to indicate that other strings are acceptable as input.

For more information about the validation done on game ids, instance ids,
and player ids, look to utils.py.
"""
...
def run_with_response_as_transaction(req_handler, operation, *args, **kwargs):
  ...
  Runs operation as a database transaction, creates an OperationResponse
  with the return value and writes it to the request handler.

  If an exception raises to this function a traceback is written to the
  debug log and an OperationResponse is written to the request handler with the
  error message as its contents and the error key set to True.
  """
  try:
...
class OperationResponse():
  ...
  server request. When operations are specific to a game instance, the
  operation response includes information about the current state of
  that instance.

  If an error is encountered during an operation the OperationResponse
  includes only the error boolean and the error's message as its response.
  ...
  def __init__(self, response, error=False):
    ...
    Args:
      response: If no error occurs, response should be a tuple of the
        ...
        If an error is encountered, response should be an error message.
      error: A boolean indicating whether the operation encountered an error
        during execution.

    The OperationResponse's attributes are automatically filled in by
    reading the attributes of the model in the response tuple. If the model
    is a Game object then iid, leader and players are left with empty values.
    """
    self.error = error
    self.iid = ''
    self.leader = ''
  ...
  def write_to_handler(self, req_handler):
    ...
    If the 'fmt' field of this request is 'html' then the response
    is formatted to be written to the web. Otherwise, it is
    formatted to be sent as json.
    """
  ...
  def get_response_object(self, request_type):
    ...
    Args:
      request_type: The type of server request that caused this operation.
...
def get_instance_lists(gid, iid, pid):
  ...
  Args:
    ...
    iid: The instance id of the Game Instance object that this method targets.
    pid: A string containing the requesting player's email address.

  The gid and pid must be valid, but the iid can be blank. This is because
  a player must be able to query for lists of instances without being in one.

  Returns:
    A tuple containing a database model and a dictionary of instance lists.
    The database model will be a Game Instance if the gid and iid parameters
    specify a valid GameInstance, otherwise the model will be a Game.
    Instance lists are returned in the same format as
    get_instance_lists_dictionary.
...
def invite_player(gid, iid, invitee):
  ...
  Only modifies the instance if the player has not already been invited
  and has not joined the game.

  Returns:
    A tuple of the game instance and a single item dictionary:
      inv: The email address of the invited player if they are invited. If
        the player is not invited (because they have already been invited
        or have already joined the game), the value of 'inv' is the empty
        string.

  Raises:
    ValueError if the game id, iid or invitee email address are invalid.
...
def join_instance(gid, iid, pid):
  ...
  A player can join a game instance if it is not full and either
  the instance is public or the player has been invited. If this
...
def get_messages(gid, iid, message_type, recipient, count, time):
  ...
  Args:
    gid: The game id of the Game object that is a parent of the desired
      instance.
    iid: This instance id of the Game Instance to fetch messages from.
    message_type: A string 'key' for the message. If message_type is the
      empty string, all message types will be returned.
    recipient: The player id of the recipient of the messages. This operation
      will also return messages that are sent with an empty recipient field.
    count: The maximum number of messages to retrieve.
    time: A string representation of the earliest creation time of a
      message to returned. Must be in ISO 8601 format to parse correctly.

  Uses the get_messages function of the GameInstance class to retrieve
  messages.

  Returns:
    A tuple of the game instance and a dictionary with two items:
      'count': The number of messages returned.
      'messages': A list of the dictionary representations of the fetched
        messages.
  """
  utils.check_gameid(gid)
  utils.check_instanceid(iid)
...
def new_instance(gid, iid_prefix, pid):
  ...
  Args:
    ...
    iid_prefix: The desired instance id. If no instance has been made with
      this name before, then this will be the instance id of the newly
      created instance. However, since instance ids must be unique, the
      actual instance id will likely be iid_prefix with a number suffix.
    pid: The id of the first player and leader of the game.

  The instance id will start with iid_prefix, but could have any suffix. If the
  parent Game object does not exist, it will automatically be created.

  Returns:
    A tuple of the newly created instance and an instance lists dictionary
    (see get_instance_lists_as_dictionary).
...
def new_message(gid, iid, pid, message_type, message_recipients,
  ...
  Returns:
    A tuple of the specified game instance and a dictionary with two items:
      'count' : The number of messages created.
      'mrec' : The list of email addresses that were sent messages.

  Raises:
    ValueError if the requesting player or any of the message recipients
      are not members of the specified game instance.
  """
  utils.check_gameid(gid)
  utils.check_instanceid(iid)
  instance = utils.get_instance_model(gid, iid)
...
def server_command(gid, iid, pid, command, arguments):
  ...
  Args:
    ...
    iid: The instance id of the GameInstance model for this operation.
  ...
  Unless the dynamic property do_not_put has been set to False, this will
  put the database model after the command has been performed. This
  means that server commands do not need to make intermediate puts
  of the instance model passed to them.

  Returns:
    A tuple of the model used in the server command's execution and a
    two item dictionary:
      'type': The requested command key.
      'contents': A Python value of the response value of the command. This
        varies among server commands but must always be able to be encoded
        to JSON.
...
def set_leader(gid, iid, pid, leader):
  ...
  Returns:
    A tuple of the change game instance model and a dictionary with two items:
  ...
  Raises:
    ...
    ValueError if player or leader are in the specified game instance.
  """
  utils.check_gameid(gid)
  utils.check_instanceid(iid)
  instance = utils.get_instance_model(gid, iid)
...
def get_instance(gid, iid):
  ...
  Args:
    gid: The game id of the desired GameInstance object's parent Game object.
    iid: The instance id of the desired GameInstance object.

  Returns:
    A tuple of the game instance object and its dictionary representation.
...
def get_instances_lists_as_dictionary(game, player):
  ...
  Args:
    game: The Game database model that is the parent of the instances to query.
    player: The email address of the player to get instance lists for.

  Returns:
    A dictionary of lists:
      'joined' : The list of instance ids of all all instances that the
        player has joined and not subsequently left.
      'invited' : The list of instance ids of all instances that the player
        has been invited to and not yet joined.
  """
...
def get_instances_invited(game, player):
  ...
  Returns:
    An empty list if game is None. Else, returns a list of the instance
    ids of all instances with game as their parent that have player in their
    invited list.
  """
  if game is None:
    return []
...
class MainPage(webapp.RequestHandler):
  ...
  def get(self):
    ...
    self.response.out.write('<h1>Game Server for App Inventor Game Client'
        'Component</h1>')
...
class GetInstanceLists(webapp.RequestHandler):
  ...
  def post(self):
    ...
    Request parameters:
      ...
      iid: The instance id of the game instance to execute the command with.
        This is optional for this command, although, including it will result
        in the ResponseObject including leader and player information.
      pid: The player id of the requesting player.
    """
...
class GetMessages(webapp.RequestHandler):
  ...
  def post(self):
    ...
    Request parameters:
      ...
      type: The type of messages requested or the empty string to retrieve
        all messages.
      count: An integer number of messages to retrieve. This is treated as a
        maximum and defaults to 1000 if there is a failure retrieving the
        count parameter.
      mtime: A string in ISO 8601 date format. All messages returned will
        have a creation time later than this time. Defaults to datetime.min
        if there is a failure in retrieving or parsing the parameter.
    """
...
class InvitePlayer(webapp.RequestHandler):
  ...
  def post(self):
    ...
    Request parameters:
      gid: The game id of the parent Game.
      iid: The instance id of the game instance to invite the player to.
      pid: The player id of the requesting player.
...
class NewInstance(webapp.RequestHandler):
  ...
  def post(self):
    ...
    Request parameters:
      gid: The game id of the parent Game.
      iid: The proposed instance id of the new instance. The instance id
        of the created instance could differ from this if the proposed id
        is already in use.
      pid: The player id of the requesting player.
    """
    ...
    iid = self.request.get(INSTANCE_ID_KEY)
    pid = self.request.get(PLAYER_ID_KEY)
    run_with_response_as_transaction(self, new_instance, gid, iid, pid)
...
class ServerCommand(webapp.RequestHandler):
  ...
  def post(self):
    ...
    Request parameters:
      gid: The game id of the parent Game to execute the command with.
      iid: The instance id of the game instance to execute the command with.
      pid: The player id of the requesting player.
      command: The key of the command.
      arguments: Json representation of the arguments to the server command.
    """
...
class SetLeader(webapp.RequestHandler):
  ...
  def post(self):
    ...
    Request parameters:
      gid: The game id of the parent Game.
      iid: The instance id of the game instance to change the leader of.
...
class GetInstance(webapp.RequestHandler):
  ...
  def post(self):
    ...
    Request parameters:
      gid: The game id of the parent Game.
      iid: The instance id of the game instance to get the information of.
    """
...
def application(custom_command_dict):
  ...
  Args:
    custom_command_dict: A dictionary of command name strings to functions.

  The custom_command_dict will be added to the server's command dictionary
  so that custom commands can be invoked with the ServerCommand request
  handler. If command names in custom_command_dict are the same as built in
  server commands they will overwrite the built in functions.
  """
